...
<div class="grids">
                ...
                <nav>             
                        ...
                        <div id="block_bg" class="block">   
							...
							<form action="visits.php" method="POST" enctype="multipart/form-data">
                            <table>
                            <tr>
							
							 <td><label><b>Doctor assigned</b></label></td>
                            
                              <td>  <select name="staff" id="staff" onchange="OnSelectionChange(this)" class="form-control" />
                                                <?php
                                                $sql28="SELECT firstname, lastname FROM staff";
                                                $records4=mysqli_query($db,$sql28);
                                                   while($users4=mysqli_fetch_array($records4,MYSQLI_ASSOC))
                                                        {
                                                            echo "<option>".$users4['firstname']." ".$users4['lastname']."</option>";
														$countyname=$users4['firstname'].$users4['lastname'];
														
                                                        }

                                                ?></select></td>
                            <td><label><b>Payment Type</b></label></td>
                            <td>
                                ...
                                <input type="hidden" name="p_id" value="<?php echo $p_id ?>" id="p_id" class="form-control">
                             <td><input type="submit" name="register" value="Save Record" class="btn btn-success"></td>
                             </tr>
                                <br>
    ...
    <p>
                    ...
                    $sql ="SELECT patient_id, visit_id, v_date, firstname,lastname,insurance_name from visits join staff join med_insurance on staff.staff_id = visits.staff_assigned WHERE visits.Payment_Type = med_insurance.insurance_id and visits.patient_id = '$p_id'";
                    ...
                    $id = $row['patient_id'];
                        ...
                        ?>
                                                ...
                                                <td><center><?php echo $row['visit_id']; ?></center></td>
...
<?php
  //  mysqli_select_db('pms',mysqli_connect($db,'localhost','root',''))or die(mysqli_error($db));
	...
	$xx1 = substr($xx, 0, 3);
	$xx2 = substr($xx, -3);
        $sql1="SELECT * FROM staff WHERE  firstname like '$xx1%' and lastname like'%$xx2'";
        $user_query=mysqli_query($db,$sql1) or die("error getting data");
        while($row = mysqli_fetch_array($user_query, MYSQLI_ASSOC)){
